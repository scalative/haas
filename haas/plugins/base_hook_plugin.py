...
class BaseHookPlugin(IHookPlugin):
    ...
    @classmethod
    def _get_enabling_option_string(cls, option_prefix, dest_prefix):
        name = option_prefix.strip('-')
        enabling_option = '--with-{0}'.format(name)
        enabling_dest = 'with_{0}'.format(dest_prefix)
        return name, enabling_option, enabling_dest

    @classmethod
    def add_parser_arguments(cls, parser, option_prefix, dest_prefix):
        name, enabling_option, enabling_dest = cls._get_enabling_option_string(
            option_prefix, dest_prefix)
